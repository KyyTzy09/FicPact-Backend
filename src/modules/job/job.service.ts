...
export class JobService {
  ...
  public async createReflectionTrigger() {
    ...
    const users = await this.userRepository.findUsersByNextReflectionDate(
      now,
      tenMinutesLater,
    );
    ...
    const mappedExistingNotificationUserIds = new Map(
      existingNotifications.map((notification) => [notification.userId, true]),
    );
  ...
  public async whatshappNotification() {
    ...
    const userIds = users.filter((user) => user.phone).map((user) => user.id);

    const twoHoursLater = new Date(now);
    twoHoursLater.setHours(now.getHours() + 2);

    const quests = await this.questRepository.findPendingQuestsBetweenDates(
      userIds,
      now,
      twoHoursLater,
    );
  ...
  public async createFailedQuestNotification() {
    ...
    const quests = await this.questRepository.findFailedQuestsBeforeDate(
      now,
      twoHoursAgo,
    );
