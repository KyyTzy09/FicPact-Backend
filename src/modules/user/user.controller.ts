...
export const userController = new Hono()
  ...
  .get(
    ...
    describeRoute({
      tags: ["User"],
      summary: "Check if User is First Reflection",
      security: [{ bearerAuth: [] }],
    }),
    authMiddleware,
    async (c) => {
      const userId = c.get("user").id;
      const result = await userService.isFirstReflection(userId);
      return HttpResponse(
        c,
        200,
        "First reflection status retrieved successfully",
        result,
      );
    },
  )
  .patch(
    ...
    async (c) => {
      ...
      const result = await userService.updateReflectionTime(
        userId,
        days,
        hours,
      );
      return HttpResponse(
        c,
        200,
        "Reflection time updated successfully",
        result,
      );
    },
  );
