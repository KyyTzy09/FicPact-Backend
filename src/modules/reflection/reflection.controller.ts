...
import {
  createQuestReflectionValidation,
  createReflectionValidation,
...
export const reflectionController = new Hono()
  .get("/latest", authMiddleware, async (c) => {
  ...
  .post(
    ...
    describeRoute({
      tags: ["Reflection"],
      summary:
        "Create Quest reflection (memiliki 2 type, type quest gagal dan sukses)",
      security: [{ bearerAuth: [] }],
    }),
    ...
    async (c) => {
      ...
      return HttpResponse(
        c,
        201,
        "Quest reflection created successfully",
        result,
      );
    },
  )
  ...
  .post(
    ...
    authMiddleware,
    validator("json", createReflectionValidation),
    async (c) => {
      const userId = c.get("user").id;
      const { notificationId } = c.req.valid("json")
      const result = await reflectionService.CreateUserReflection(userId, notificationId);
      return HttpResponse(c, 200, "Reflection retrieved successfully", result);
    },
  );
