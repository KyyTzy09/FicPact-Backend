...
export class PunishmentService {
  constructor(
    private readonly punishmentRepository: PunishmentRepository,
  ...
  async getQuestPunishment(questId: string, userId: string) {
    ...
    const existingPunishment =
      await this.punishmentRepository.findPunishmentByUserId(questId, userId);
    if (!existingPunishment)
      throw new HTTPException(404, { message: "Hukuman tidak ditemukan" });
  ...
  async createPunishment(
    ...
    const createdPunishment = await this.punishmentRepository.createPunishment(
      questId,
      userId,
      name,
      deadlineAt,
    );
    if (!createdPunishment)
      throw new HTTPException(400, { message: "Gagal membuat hukuman" });
  ...
  async updatePunishmentStatus(
    ...
    const existingPunishment =
      await this.punishmentRepository.findPunishmentByUserId(
        punishmentId,
        userId,
      );
    if (!existingPunishment)
    ...
    const punishmentStatus: QuestPunishmentStatus = status
      ? QuestPunishmentStatus.COMPLETED
      : QuestPunishmentStatus.FAILED;
    ...
    const updatedPunishment =
      await this.punishmentRepository.updatePunishmentStatus(
        punishmentId,
        punishmentStatus,
      );
    ...
    if (!updatedPunishment)
      throw new HTTPException(400, { message: "Gagal memperbarui hukuman" });
