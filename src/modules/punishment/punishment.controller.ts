...
import {
  createPunishmentValidation,
  updatePunishmentStatusValidation,
} from "./punishment.validation.js";
...
export const punishmentController = new Hono()
  .get(
    ...
    async (c) => {
      ...
      const result = await punishmentService.getQuestPunishment(
        punishmentId,
        userId,
      );
  ...
  .post(
    ...
    async (c) => {
      ...
      const result = await punishmentService.createPunishment(
        questId,
        userId,
        name,
        new Date(deadlineAt),
  ...
  .patch(
    ...
    async (c) => {
      ...
      const userId = c.get("user").id;
      const { status, notificationId } = c.req.valid("json");
      const result = await punishmentService.updatePunishmentStatus(
        userId,
        punishmentId,
        status,
        notificationId,
      ...
      return HttpResponse(
        c,
        200,
        "Punishment status updated successfully",
        result,
      );
    },
  );
